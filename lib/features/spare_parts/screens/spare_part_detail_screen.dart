...
/// View / edit / delete screen for a single Spare Part.
/// Accepts the item id, loads the full record from the API.
class SparePartDetailScreen extends StatefulWidget {
  final int itemId;
  const SparePartDetailScreen({Key? key, required this.itemId})
      : super(key: key);
...
class _SparePartDetailScreenState extends State<SparePartDetailScreen> {
  ...
  Future<void> _loadItem() async {
    setState(() { _loadingItem = true; _loadError = null; });
    try {
      final item = await _api.getSparePartById(widget.itemId);
      if (mounted) {
        setState(() {
          _item = item;
          _nameController.text = item.sparepartsname;
          _partNumberController.text = item.partnumber;
          _dateController.text = item.date;
          _notesController.text = item.spnotes;
          _quantity = item.spquantity;
          _loadingItem = false;
        });
      }
    } on ApiException catch (e) {
      if (mounted) setState(() { _loadError = e.message; _loadingItem = false; });
    } catch (e) {
      if (mounted) setState(() { _loadError = e.toString(); _loadingItem = false; });
    }
  }
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      await _api.updateSparePart(widget.itemId, {
  ...
  Future<void> _confirmDelete() async {
    final name = _item?.sparepartsname ?? 'this spare part';
    ...
    try {
      await _api.deleteSparePart(widget.itemId);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
